...
fn both_edges_u128() {
    ...
    let mut p = crate::CROCKFORD.encode_u128(0);
    let d = crate::CROCKFORD.decode_u128(p.as_bytes()).unwrap();
    assert_eq!(0, d, "mismatch decode for 0: {p} vs {d}");
    ...
    for r in rs {
        for n in r {
            ...
            assert!(format!("{e:0>25}") > format!("{p:0>25}"), "next encoded {e} not greater than prev {p}");
            p = e;
        }
    }
}
